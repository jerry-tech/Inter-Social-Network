import * as tslib_1 from "tslib";
import { Component } from '@angular/core';
import { Validators } from '@angular/forms';
function passwordMatcher(c) {
    const passwordControl = c.get('password');
    const confpasswordControl = c.get('confpassword');
    if (passwordControl.value === confpasswordControl.value) {
        return null;
    }
    return { 'match': true };
}
let AdminComponent = class AdminComponent {
    constructor(router, route, dataService, feedService, fb) {
        this.router = router;
        this.route = route;
        this.dataService = dataService;
        this.feedService = feedService;
        this.fb = fb;
        this.allJobs = [
            'Admin', 'Counter Officer', 'Cleaner', 'Driver', 'Mechanic'
        ];
        this.formResponse = [{
                fontawesome: "fa fa-check-circle",
                message: "User Account Created Successfully"
            }];
        this.errorData = [{
                fontawesome: "fa fa-times-circle",
                message: "Email Address Already Exist or Error Creating Account"
            }];
    }
    ngOnInit() {
        this.route.paramMap.subscribe((paramMap) => {
            this.isLoading = true; //loading spinner
            this.feedService.getFlagged().subscribe(res => {
                this.isLoading = false; //loading spinner
                this.flaggedPost = res.data;
                console.log(res.data);
            }, err => console.log(err));
        });
        // this.feedService.getFlagged().subscribe(
        //   res => {
        //     this.flaggedPost = res.data;
        //     console.log(res.data);
        //   },
        //   err => console.log(err)
        // )
        this.feedService.getAllUsers().subscribe(res => {
            this.isLoading = false; //loading spinner
            this.allUsers = res;
            console.log(this.allUsers);
        }, err => (console.log(err)));
        //reative form validations 
        this.adminRegister = this.fb.group({
            firstName: ['', [Validators.required, Validators.maxLength(50)]],
            lastName: ['', [Validators.required, Validators.maxLength(50)]],
            email: ['', [Validators.required, Validators.email]],
            passwordGroup: this.fb.group({
                password: ['', [Validators.required, Validators.minLength(6)]],
                confpassword: ['', [Validators.required, Validators.minLength(6)]],
            }, { validator: passwordMatcher }),
            gender: ['', Validators.required],
            jobrole: ['', Validators.required],
            dept: ['', Validators.required],
            address: ['', [Validators.required, Validators.minLength(5)]],
            userImage: ['', Validators.required],
        });
    }
    //check if a post is flagged
    isChecked(flag) {
        let valid = true;
        if (flag != 't') {
            valid = false;
        }
        return valid;
    }
    //unflag A post
    flagPost(id, value) {
        this.isLoading = true; //loading spinner
        console.log(id, value);
        this.feedService.flagAPost(id, value).subscribe(() => {
            const updatedFlag = this.flaggedPost.filter(post => post.article_id == id);
            this.isLoading = false; //loading spinner
            this.flaggedPost = updatedFlag;
        }, err => console.log(err));
    }
    //image preview
    onImagePicked(event) {
        const file = event.target.files[0];
        this.adminRegister.patchValue({ userImage: file });
        this.adminRegister.get('userImage').updateValueAndValidity();
        const reader = new FileReader();
        reader.onload = () => {
            this.previewImage = reader.result;
        };
        reader.readAsDataURL(file);
        this.selectedImage = file;
    }
    //to submit form
    save() {
        this.isLoading = true; //loading spinner
        this.feedService.registerUsers(this.adminRegister.value.firstName, this.adminRegister.value.lastName, this.adminRegister.value.email, this.adminRegister.controls['passwordGroup'].value.password, this.adminRegister.value.gender, this.adminRegister.value.jobrole, this.adminRegister.value.dept, this.adminRegister.value.address, this.selectedImage).subscribe(res => {
            if (res.status === 'success') {
                this.isLoading = false; //loading spinner
                document.getElementById('clickAdmin').style.display = 'block';
                console.log(res);
            }
            else {
                document.getElementById('errorAdmin').style.display = 'block';
            }
        }, err => console.log(err));
        console.log(this.adminRegister.value.gender);
    }
    //radio button toggle values using reative form set method
    setGender(gender) {
        const genderControl = this.adminRegister.get('gender');
        if (gender === 'Male') {
            genderControl.setValidators(Validators.required);
        }
        else {
            genderControl.clearValidators();
        }
        genderControl.updateValueAndValidity();
    }
    toggleLinks() {
        document.getElementById('rightcont').style.display = 'none';
        document.getElementById('account').style.display = 'block';
        document.getElementById('activeusers').style.display = 'none';
    }
    togglePost() {
        document.getElementById('rightcont').style.display = 'block';
        document.getElementById('account').style.display = 'none';
        document.getElementById('activeusers').style.display = 'none';
    }
    toggleUser() {
        document.getElementById('activeusers').style.display = 'block';
        document.getElementById('rightcont').style.display = 'none';
        document.getElementById('account').style.display = 'none';
    }
    toggleRegForm() {
        document.getElementById('activeusers').style.display = 'none';
        document.getElementById('rightcont').style.display = 'block';
        document.getElementById('account').style.display = 'block';
    }
    //opening material dialog
    openDialog() {
        // const dialogConfig = new MatDialogConfig();
        // this.matDialog.open(DialogBodyComponent, dialogConfig);
    }
};
AdminComponent = tslib_1.__decorate([
    Component({
        selector: 'app-admin',
        templateUrl: './admin.component.html',
        styleUrls: ['./admin.component.css']
    })
], AdminComponent);
export { AdminComponent };
//# sourceMappingURL=admin.component.js.map